...
public class DBDbHelper extends SQLiteOpenHelper {
    ...
    private static final String SQL_CREATE_ENTRIES =
            "CREATE TABLE " + DBContract.DB.TABLE__OOSOBY + " ( " +
                    DBContract.DB._ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " +
                    DBContract.DB.COLUMN_IMIE + TEXT_TYPE + COMMA_SEP +
                    DBContract.DB.COLUMN_DATE + DATE_TYPE + COMMA_SEP +
                    DBContract.DB.COLUMN_IMAGE_PATH + TEXT_TYPE + COMMA_SEP+
                    DBContract.DB.COLUMN_WIEK + INT_TYPE + " );";
    private static final String SQL_DELETE_ENTRIES = "DROP TABLE IF EXISTS " + DBContract.DB.TABLE__OOSOBY;
